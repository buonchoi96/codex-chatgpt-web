...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      "For native desktop automation, use a Computer Use capability that is actually exposed by the current outer Codex tool registry. Do not treat ChatGPT's browser-only computer surface as proof of native desktop access.",
      "On Windows native-app tasks, explicitly search for node_repl before concluding that desktop control is unavailable. When mcp__node_repl__js is available, prefer the official Computer Use path through persistent node_repl and @oai/sky: import @oai/sky, retain sky in the REPL session, call sky.list_apps(), then continue with the native app/window operations exposed by sky.",
      "Treat cua_repl as browser-oriented unless its current tool description or runtime state explicitly proves that native computer APIs are enabled. If cua_repl reports apps=[], 'Native computer APIs are disabled', a missing sky trusted service, or an equivalent native-surface failure, do not use that result to conclude that Windows desktop control is unavailable; try node_repl + @oai/sky.",
      "Discover the exact callable surface through Codex Native tool inventory/discovery and invoke that exact outer-Codex tool through Codex Native; do not assume a third-party MCP namespace or backend.",
      "The codex_windows_computer_use_* bridge tools are legacy compatibility helpers only. Do not call them unless the current native tool inventory has already proved that the corresponding mcp__windows_computer_use tools are loaded in this exact outer Codex turn.",
      "If neither a working node_repl/@oai/sky native path nor another explicitly native-capable official Computer Use surface is available, report that exact limitation instead of substituting an unrelated browser-only surface.",
      ...
      ...(!manualControl ? [
        "For Full Harness execution, the mandatory completion receipt is the dedicated codex_turn_complete tool. Do not emit a final answer until codex_turn_complete has accepted a receipt with remaining_actionable_requirements empty. If it rejects the receipt, continue working.",
      ] : []),
      "Continue using the available tools until the requested work is complete and verified.",
      ...(!manualControl ? [
        "Write the user-facing final answer only after the last required tool result has settled, the dedicated codex_turn_complete tool has accepted the full-task receipt, and the full-request completion check above passes. Do not call another work tool after beginning that final answer.",
      ] : [
