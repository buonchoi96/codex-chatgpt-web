...
export function compileChatGptWebPrompt(
  ...
  const transportContract = parsed._compactionRequest
    ...
    ? [
      ...
      "Prefer official OpenAI Codex Computer Use or Unified Computer Use capabilities when they are available. Discover the exact callable surface through Codex Native tool inventory/discovery and invoke that exact outer-Codex tool through Codex Native; do not assume a third-party MCP namespace or backend.",
      "The codex_windows_computer_use_* bridge tools are legacy compatibility helpers only. Use them only when the corresponding legacy Windows Computer Use tools are genuinely loaded in the current outer Codex turn.",
      "If the outer Codex registry exposes no suitable Computer Use capability, report that exact limitation instead of substituting an unrelated browser-only surface.",
