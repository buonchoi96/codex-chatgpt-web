    ...
    if (message.type === "event") {
      if (message.event === "heartbeat") pending.turn.onHeartbeat?.();
        ...
        void Promise.resolve().then(() => prepare?.()).then(prepared => {
          ...
          if ((prepared.archive || prepared.contextFile) && !this.helperFeatures.has("context-archive")) {
            throw new Error("Launcher browser helper does not support context archive transport; update or restart the launcher");
          }
