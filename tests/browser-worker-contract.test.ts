...
test("only retryable transport failures after proven tool progress retain an active recovery conversation", () => {
  const progress = {
    snapshot: () => ({ revision: 4, lastToolBatchRevision: 3, activeToolCalls: 0, lastProgressAt: Date.now() }),
    waitForChange: async () => { throw new Error("unused"); },
    acknowledgeToolBatch: async () => {},
  };
  const turn = { conversationKey: "a".repeat(64), retainConversationOnRetry: true, externalProgress: progress };
  expect(chatGptRetryableFailureCanRetainConversation(
    turn,
    new ChatGptWebAdapterError("transient", {
      status: 502, errorType: "server_error", code: "upstream_server_error", retryable: true,
    }),
  )).toBeTrue();
  expect(chatGptRetryableFailureCanRetainConversation(
    turn,
    new ChatGptWebAdapterError("rebind", {
      status: 502, errorType: "server_error", code: "chatgpt_response_page_rebind_failed", retryable: true,
    }),
  )).toBeTrue();
  expect(chatGptRetryableFailureCanRetainConversation(
    turn,
  ...
  expect(chatGptRetryableFailureCanRetainConversation(
    { ...turn, externalProgress: { ...progress, snapshot: () => ({ revision: 0, lastToolBatchRevision: 0, activeToolCalls: 0 }) } },
    new ChatGptWebAdapterError("transient", {
      status: 502, errorType: "server_error", code: "upstream_server_error", retryable: true,
    }),
  )).toBeFalse();
...
test("completion receipt settlement gets a bounded grace window before recovery", async () => {
  ...
  expect(calls).toBe(2);
  expect(await waitForChatGptCompletionReceipt(async () => false, 0)).toBeFalse();
});

test("safety-blocked final outcomes are terminal without waiting through receipt recovery", () => {
  expect(chatGptFinalIndicatesSafetyBlocked("WINDOWS_CU_FOREGROUND_TRANSITION_SAFETY_BLOCKED")).toBeTrue();
  expect(chatGptFinalIndicatesSafetyBlocked("This tool call was blocked by OpenAI's safety checks.")).toBeTrue();
  expect(chatGptFinalIndicatesSafetyBlocked("Unable to continue because the tool was blocked by safety checks.")).toBeTrue();
  expect(chatGptFinalIndicatesSafetyBlocked("WINDOWS_CU_FOREGROUND_TRANSITION_E2E_OK")).toBeFalse();
  expect(chatGptFinalIndicatesSafetyBlocked("Safety checks were discussed, but the task completed successfully.")).toBeFalse();
});

test("developer MCP capability loss is classified separately from ordinary missing receipts", () => {
  expect(chatGptFinalIndicatesDeveloperMcpUnavailable(
    "FORBIDDEN: This conversation does not support developer MCPs",
  )).toBeTrue();
  expect(chatGptFinalIndicatesDeveloperMcpUnavailable(
    "Codex Native developer MCP access must be restored for this conversation.",
  )).toBeFalse();
  expect(chatGptFinalIndicatesDeveloperMcpUnavailable(
    "Developer MCPs are available and the task completed.",
  )).toBeFalse();
});
...
test("missing native completion receipt gets a progress-aware bounded same-turn continuation prompt", () => {
  ...
  expect(prompt).toContain("bridge rejected that final-answer boundary");
...
test("completion receipt recovery resets its stalled budget whenever Codex Native makes progress", () => {
  let state = { recoveries: 0, noProgressRecoveries: 0, progressRevision: 0 };

  let next = advanceChatGptCompletionReceiptRecovery(state, 0);
  expect(next.allowed).toBeTrue();
  expect(next.progressed).toBeFalse();
  expect(next.noProgressRecoveries).toBe(1);

  state = next;
  next = advanceChatGptCompletionReceiptRecovery(state, 0);
  expect(next.allowed).toBeTrue();
  expect(next.noProgressRecoveries).toBe(2);

  next = advanceChatGptCompletionReceiptRecovery(next, 7);
  expect(next.allowed).toBeTrue();
  expect(next.progressed).toBeTrue();
  expect(next.noProgressRecoveries).toBe(0);
  expect(next.progressRevision).toBe(7);
...
test("an accepted turn survives internal observation faults instead of being torn down", () => {
  ...
  expect(CHATGPT_EXTERNAL_PROGRESS_STALL_CEILING_MS).toBe(15 * 60_000);
...
test("silent running turns become retryable stalls while real progress resets the watchdog", () => {
  const tracker = new ChatGptRunningProgressTracker(5_000);
  const base = {
    running: true,
    visibleText: "",
  ...
  expect(tracker.update(base, 5_999)).toBeFalse();
  expect(tracker.update(base, 6_000)).toBeTrue();

  expect(tracker.update({ ...base, visibleText: "progress" }, 6_001)).toBeFalse();
  expect(tracker.update({ ...base, visibleText: "progress" }, 10_999)).toBeFalse();
  expect(tracker.update({ ...base, visibleText: "progress", externalToolCallsInFlight: true }, 11_000)).toBeFalse();
